...
io.on("connection", (socket) => {
    ...
    const clientIp = socket.request.headers["x-forwarded-for"]?.split(",")[0] ||
        socket.request.socket.remoteAddress ||
        "Unknown";
    const { location, country, flag } = getLocationInfo(clientIp);
    const color = generateColor();
    // Handle session restoration
    const authSessionId = socket.handshake.auth.sessionId || null;
    let userSession;
    ...
    socket.on("cursor-change", (data) => {
        const user = sessions.get(Array.from(sessions.entries()).find(([_, u]) => u.socketId === socket.id)?.[0] || "");
        if (user) {
    ...
    socket.on("update-user", (data) => {
        const sessionKey = Array.from(sessions.entries()).find(([_, u]) => u.socketId === socket.id)?.[0];
        if (sessionKey) {
    ...
    if (io.engine.clientsCount > 100) {
        socket.emit("warning", {
            message: "Server is experiencing high load. Some features may be limited.",
        });
    }
});
